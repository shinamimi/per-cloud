 ...
 *
 * 修改指引：
 * - 【习惯】修改 requestId       → Long requestId；好友请求记录 id，接受/拒绝接口的路径参数
 *                         （PUT /api/friends/requests/{id}/accept、/reject）
 * - 【习惯】修改 fromUserId / fromUsername / fromNickname / fromAvatar → 发起方资料（请求记录 + 对方资料），
 *                         仅展示用，改动影响前端请求列表渲染
 * - 【习惯】修改 status          → FriendRequestStatus status；自定义枚举（enums/FriendRequestStatus.java）：
 *                         PENDING / ACCEPTED / REJECTED；待处理列表仅返回 PENDING 项，改动取值需同步状态机
