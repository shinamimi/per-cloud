 ...
 *
 * 修改指引：
 * - 【习惯】修改 instant         → boolean instant；true 表示秒传完成，false 表示哈希未命中需走 upload/init 分片上传；
 *                         前端据此决定是否发起实际上传，改动该字段名/语义需同步前端分叉逻辑
 * - 【习惯】修改 file            → FileNodeResponse file；instant=true 时为新建文件记录，false 时为 null；
 *                         前端避免在 miss 时读取 file 字段（防 NPE）
 */
@Data
