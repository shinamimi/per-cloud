 ...
 *
 * 修改指引：
 * - 【统一】修改 type            → String type；取值 IMAGE / VIDEO / AUDIO / PDF / TEXT / UNSUPPORTED，前端按此选择预览组件；
 *                         新增类型需同步 PreviewService 判定逻辑与前端组件映射；改后需同步 PreviewService 判定逻辑与前端组件映射
 * - 【统一】修改 url             → String url；直接预览地址（presigned，10 分钟有效），过期后需重新请求预览接口；改后需同步前端过期重取策略与预览地址生成
