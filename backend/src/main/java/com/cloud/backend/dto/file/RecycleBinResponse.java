 ...
 *
 * 修改指引：
 * - 【习惯】修改 id              → Long id；回收站记录 id，恢复/彻底删除接口的路径参数（POST /api/files/recycle-bin/{id}/restore、
 *                         DELETE /api/files/recycle-bin/{id}）
 * - 【习惯】修改 fileId          → Long fileId；原文件 id
 * - 【习惯】修改 originalName    → String originalName；回收前原始文件名，恢复时可能因同名冲突自动追加后缀
 * - 【习惯】修改 type            → Integer type；0-文件 1-目录（t_recycle_bin.type TINYINT），恢复时决定建文件还是建目录
