 ...
 *
 * 修改指引：
 * - 【习惯】修改单位             → captcha/loginAttempt/blacklist/filePreview 单位为秒，downloadLinkMinutes 单位为分钟（presigned URL 有效期）；
 *                          单位混用易错，改动需同步配置读写逻辑与前端表单的单位标注
 ...
 * - 【习惯】新增缓存项            → 新增字段并同步缓存读写逻辑与前端，否则该配置不生效
 */
@Data
