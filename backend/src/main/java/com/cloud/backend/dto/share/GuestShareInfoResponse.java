 ...
 *
 * 修改指引：
 * - 【习惯】修改 shareToken      → String shareToken；分享 token（10 位去混淆短码，唯一标识），前端据此拼装分享链接；
 *                         改动生成规则需同步 ShareTokenGenerator，历史链接会失效
 ...
 *                         前端据此展示"已过期/已取消/已达上限"等
 * - 【习惯】修改 requirePassword → Boolean requirePassword；true 时前端弹提取码框；未验证时本接口也返回，文件树/下载需先验证
 * - 【习惯】修改 allowDownload / allowSave → 是否允许下载/转存，前端据此隐藏下载/转存按钮
 * - 【习惯】修改 maxDownload / downloadCount → 下载次数上限/已下载次数（maxDownload=0 表示不限）；
 *                         downloadCount 达到上限时服务端置 EXHAUSTED
