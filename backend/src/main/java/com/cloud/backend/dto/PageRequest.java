 ...
 *
 * 修改指引：
 * - 【习惯】修改字段名/类型       → page/size 为各分页接口入参，改动需同步所有继承/使用处与前端分页参数
 * - 【习惯】修改默认分页值        → page 默认 1、size 默认 20；改动影响未显式传参的接口返回条数
 * - 【习惯】修改分页语义          → page 从 1 起（page=1 时 offset=0），改动破坏与前端分页器约定，需同步所有调用处
 * - 【习惯】修改 pageSize 上限    → 当前未设上限；如需防超大 size 需在此新增校验，否则影响数据库查询压力
 * - 【习惯】修改 offset 计算      → getOffset() = (page-1)*size；改动影响所有依赖它的 SQL 分页查询
 */
