 ...
 *
 * 修改指引：
 * - 【习惯】修改响应字段名        → records/total/page/size 为列表接口统一分页响应结构，改动影响所有使用 Page 的列表接口与前端分页器
 * - 【习惯】修改分页语义          → page 从 1 开始，size 为每页条数；改起始页码会破坏前端分页器对齐，需同步所有分页接口与前端
 * - 【习惯】修改 total 含义       → 当前为符合条件的总记录数（非当前页条数），用于计算总页数；改动影响前端分页器显示
