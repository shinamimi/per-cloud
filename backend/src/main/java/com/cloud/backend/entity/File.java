 ...
 *
 * 修改指引：
 * - 【习惯】修改 id / userId / teamId / parentId → Long id（t_file.id 主键）/ Long userId（user_id）/ Long teamId（team_id，
 *                            0=个人空间 >0=团队空间）/ Long parentId（parent_id，0=根目录）；改列名需同步 DDL
 * - 【习惯】修改 name / path     → String name（t_file.name）/ String path（t_file.path 完整路径）；唯一索引
 *                            uk_user_parent_name(user_id,parent_id,name,team_id) 约束同空间同目录下 name 唯一，
 *                            改字段名或放宽唯一性需同步 DDL（业务层重名自动加后缀）
 * - 【习惯】修改 size            → Long size；对应 t_file.size，单位字节，影响配额 used_space 计算与展示
 * - 【习惯】修改 mimeType / extension → String mimeType（t_file.mime_type）/ String extension（t_file.extension）；仅展示与下载判断
 * - 【习惯】修改 fileHash        → String fileHash；对应 t_file.file_hash（SHA256），秒传索引（t_file_hash）与对象禁用
 *                            （t_disabled_object）判定的依据
 * - 【习惯】修改 isDirectory / type → Integer isDirectory（t_file.is_directory，兼容旧字段）/ FileType type（t_file.type TINYINT，
 *                            FILE=0/DIRECTORY=1，见 enums/FileType.java，按 ordinal 存库）；isDir() 兼容两者，改枚举见 FileType 修改指引
 * - 【习惯】修改 category        → Integer category；对应 t_file.category（TINYINT），0-图片 1-文档 2-视频 3-音频 4-压缩包 5-其他，
 *                            搜索按类型过滤
 * - 【习惯】修改 objectName      → String objectName；对应 t_file.object_name，MinIO 对象路径（仅文件有值）；
 *                            改它会导致下载/删除定位不到物理对象
 * - 【习惯】修改 status          → FileStatus status；对应 t_file.status（TINYINT），DELETED=0/NORMAL=1/DISABLED=2
