 ...
 *
 * 修改指引：
 * - 【习惯】新增枚举值      → 在枚举末尾追加常量（value 取未占用的中间档位）；TeamMemberRoleTypeHandler 按 value 存取、
 *                     不依赖 ordinal，取中间档位不影响存量数据，同时需在团队权限判断处补充新档位逻辑
 * - 【习惯】重命名枚举值    → 修改常量名；DB 按 value 存储不受影响，同步修改引用处即可
 * - 【习惯】修改 value     → 常量括号内数值；已存数据按旧 value 映射会错位（需数据迁移），且影响权限比较
 *                    （getValue() >= ADMIN 等判断）
