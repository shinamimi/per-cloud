 ...
 *
 * 修改指引：
 * - 【统一】新增枚举值        → 在枚举末尾追加常量；需同步补充好友请求状态机流转逻辑（接受/拒绝处理），
 *                       否则新状态无法到达，也无法写入 t_friend_request.status；
 *                       改后需同步好友请求状态机流转逻辑（接受/拒绝处理）与 t_friend_request.status 写入
 * - 【统一】重命名枚举值      → 修改常量名；DB 按枚举名称存储（默认 EnumTypeHandler），已存记录需一并迁移；
 *                       改后需同步 DB 存量记录迁移（按枚举名称存储）
 * - 【统一】删除枚举值        → 删除常量并清理引用；存量记录中的该名称将无法反查，fromName() 会兜底为 PENDING；
 *                       改后需同步清理引用处与 DB 存量记录
