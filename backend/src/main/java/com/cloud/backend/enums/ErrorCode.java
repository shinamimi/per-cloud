 ...
 *
 * 修改指引：
 * - 【统一】新增错误码      → 在对应模块分区追加常量（code + message）；code 需落在预留区间且唯一，
 *                      前端据 code 展示文案，新增后需同步前后端契约；改后需同步前端契约（code 唯一、前端据 code 展示文案）
 * - 【统一】修改 code      → 常量括号内数值；已发布接口勿改，前端按 code 判断成败/展示文案，改号会导致前后端不匹配；
 *                      改后需同步前端按 code 判断成败/展示文案的契约
 * - 【统一】修改 message   → 常量内文案；仅影响前端展示，改动安全；改后需同步前端展示文案
 * - 【统一】重命名常量      → 修改常量名；需同步修改所有抛出该错误码的业务引用处，不改变对外 code；
 *                      改后需同步所有抛出该错误码的业务引用处
 */
@Getter
public enum ErrorCode {
    ...
    /* ==================== 限流 (10700-10799) ==================== */
    RATE_LIMIT_EXCEEDED(10700, "请求过于频繁，请稍后再试"),

    /* ==================== 存储 ==================== */
    MINIO_ERROR(10501, "MinIO 存储异常");

    /** 【统一】改后需同步前端按 code 判断成败/展示文案的契约 */
