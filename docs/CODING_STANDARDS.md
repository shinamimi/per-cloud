...
| 原则 | 说明 |
|------|------|
| 解释"为什么"而非"是什么" | 代码本身说明"做了什么"，注释说明业务约束、历史原因、权衡取舍 |
| 全覆盖 | 类级 + 方法级全写，字段/行内尽量写 |
| 不注释显而易见 | getter/setter（Lombok 生成）不注释 |
| 注释要维护 | 改代码必须同步改注释，注释说谎比没有更糟 |
...
所有类必须有类级 Javadoc，说明**职责 + 设计思路**：

```java
/**
 * 用户服务实现 —— 处理用户 CRUD、配额计算、密码重置等业务。
 *
 * 设计思路：
 * 1. 配额使用三来源模型（base + reward + adminBonus）
 * 2. 密码重置记录操作日志（OperationLog）
 */
```

#### 1.6.4 方法级注释（全覆盖）

- 所有非 getter/setter 方法写 Javadoc，说明**业务语义、前置条件、副作用**
- 接口/公开方法：注明权限约束、目标校验（如"仅 ADMIN 可调，目标不能是 ADMIN+"）
- 复杂逻辑：多分支、有坑、算法、位运算必须注释
- 行内 `//` 用于：非显而易见的分支、空指针保护、边界条件、性能/并发考量（锁、原子操作原因）、TODO/扩展预留

#### 1.6.5 字段级注释

- 实体字段：对照数据库列，说明业务含义，枚举字段注明取值范围
- DTO 字段：对照接口契约
- 有默认值/单位陷阱的必须注明（如 `// 单位：字节`）
...
```ts
/** 上传任务状态：queued-排队中 / uploading-上传中 / done-完成 / error-失败 */
export type UploadTaskStatus = 'queued' | 'uploading' | 'done' | 'error'
```

#### 2.7.2 组件

- 顶部组件注释：**职责 + 数据流**（props 谁传、emit 给谁）
- 复杂函数说明业务逻辑
- 全部中文
